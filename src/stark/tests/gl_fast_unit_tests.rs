...
fn test_gl_add_simple() {
    ...
    assert!(
        ...
        "gl_add constraints should be satisfied"
    );
...
fn test_gl_add_overflow() {
    ...
    assert!(
        cs.is_satisfied().unwrap(),
        "gl_add with overflow should satisfy"
    );
    eprintln!(
        "gl_add overflow test passed, constraints: {}",
        cs.num_constraints()
    );
}
...
fn test_gl_sub_simple() {
    ...
    assert!(
        cs.is_satisfied().unwrap(),
        "gl_sub constraints should be satisfied"
    );
...
fn test_gl_sub_underflow() {
    ...
    assert!(
        cs.is_satisfied().unwrap(),
        "gl_sub with underflow should satisfy"
    );
    eprintln!(
        "gl_sub underflow test passed, constraints: {}",
        cs.num_constraints()
    );
}
...
fn test_gl_mul_large() {
    ...
    eprintln!(
        "gl_mul large test passed, constraints: {}",
        cs.num_constraints()
    );
}
...
fn test_gl_operations_sequence() {
    ...
    eprintln!(
        "Operation sequence test passed, total constraints: {}",
        cs.num_constraints()
    );
}
...
fn test_gl_add_light() {
    ...
    for &(a_val, b_val) in &test_cases {
        ...
        assert_eq!(
            actual, expected,
            "gl_add_light({}, {}) = {} but expected {}",
            a_val, b_val, actual, expected
        );
    }

    assert!(cs.is_satisfied().unwrap(), "CS should be satisfied");
    eprintln!(
        "gl_add_light constraints per op: ~{}",
...
fn test_gl_sub_light() {
    ...
    assert!(cs.is_satisfied().unwrap(), "CS should be satisfied");
    eprintln!(
        "gl_sub_light constraints per op: ~{}",
        cs.num_constraints() / test_cases.len()
    );
}
...
fn test_gl_mul_light() {
    ...
    assert!(cs.is_satisfied().unwrap(), "CS should be satisfied");
    eprintln!(
        "gl_mul_light constraints per op: ~{}",
        cs.num_constraints() / test_cases.len()
    );
}
...
fn test_gl_inv_light() {
    ...
    for &v_val in &test_cases {
        ...
        assert_eq!(
            actual, expected,
            "gl_inv_light({}) = {} but expected {}",
            v_val, actual, expected
        );
    }

    assert!(cs.is_satisfied().unwrap(), "CS should be satisfied");
    eprintln!(
        "gl_inv_light constraints per op: ~{}",
        cs.num_constraints() / test_cases.len()
    );
}
...
fn test_light_ops_composition() {
    ...
    use crate::stark::gadgets::gl_fast::{
        gl_add_light, gl_inv_light, gl_mul_light, gl_sub_light, GlVar,
    };
    use crate::stark::gl_u64::{fr_to_gl_u64, gl_add, gl_inv, gl_mul, gl_sub};
    use ark_r1cs_std::alloc::AllocVar;
    use ark_r1cs_std::fields::fp::FpVar;
    ...
    assert!(cs.is_satisfied().unwrap(), "CS should be satisfied");
    eprintln!(
        "Total constraints for complex expression: {}",
        cs.num_constraints()
    );
}
...
fn test_light_ops_edge_cases() {
    ...
    use crate::stark::gadgets::gl_fast::{gl_add_light, gl_mul_light, gl_sub_light, GlVar};
    use crate::stark::gl_u64::fr_to_gl_u64;
    use ark_r1cs_std::alloc::AllocVar;
    use ark_r1cs_std::fields::fp::FpVar;
