...
fn test_proof_verification(steps: usize, expected_fri_layers: usize) -> bool {
    eprintln!(
        "\n=== Testing with steps={} (expecting {} FRI layers) ===",
        steps, expected_fri_layers
    );
    ...
    assert_eq!(
        actual_fri_layers, expected_fri_layers,
        "FRI layer count mismatch! Expected {}, got {}",
        expected_fri_layers, actual_fri_layers
    );
...
fn test_fri_no_layers() {
    assert!(
        test_proof_verification(8, 0),
        "Should verify proof with 0 FRI layers"
    );
}
...
fn test_fri_one_layer() {
    ...
    assert!(
        test_proof_verification(64, 1),
        "Should verify proof with 1 FRI layer"
    );
}
...
fn test_fri_two_layers() {
    ...
    assert!(
        test_proof_verification(128, 2),
        "Should verify proof with 2 FRI layers"
    );
}
...
fn test_large_trace() {
    // Very large trace
    assert!(
        test_proof_verification(256, 3),
        "Should verify proof with 3 FRI layers"
    );
}
