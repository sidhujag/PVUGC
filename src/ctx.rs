...
impl PvugcContextBuilder {
    ...
    pub fn build_arming_pkg_hash(
        armed_bases_bytes: &[u8],
        header_metadata: &[u8],
    ) -> [u8; 32] {
        let mut hasher = Sha256::new();
...
mod tests {
    ...
    fn test_ctx_core_deterministic() {
        ...
        let ctx1 = PvugcContextBuilder::new(vk_hash, x_hash, y_cols, nonce)
            .build_ctx_core();
        let ctx2 = PvugcContextBuilder::new(vk_hash, x_hash, y_cols, nonce)
            .build_ctx_core();
    ...
    fn test_ctx_core_differs_on_nonce() {
        ...
        let ctx1 = PvugcContextBuilder::new(vk_hash, x_hash, y_cols, nonce1)
            .build_ctx_core();
        let ctx2 = PvugcContextBuilder::new(vk_hash, x_hash, y_cols, nonce2)
            .build_ctx_core();

        assert_ne!(ctx1, ctx2, "Different nonces must produce different ctx_core");
    }
    ...
    fn test_epoch_nonce_registry() {
        ...
        assert!(registry.register(nonce1).is_err(), "Duplicate nonce should be rejected");
    }
}
