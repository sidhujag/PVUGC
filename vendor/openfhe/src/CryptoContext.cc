...
{
    // OpenFHE >=1.4 expects Plaintext& (non-const) for ct±pt operations.
    // Copying the shared_ptr gives a mutable lvalue that aliases the same PlaintextImpl.
    auto pt = plaintext.GetRef();
    return std::make_unique<CiphertextDCRTPoly>(m_cryptoContextImplSharedPtr->EvalAdd(
        ciphertext.GetRef(), pt));
}
...
{
    auto pt = plaintext.GetRef();
    m_cryptoContextImplSharedPtr->EvalAddInPlace(ciphertext.GetRef(), pt);
}
...
{
    auto pt = plaintext.GetRef();
    m_cryptoContextImplSharedPtr->EvalAddInPlace(pt, ciphertext.GetRef());
}
...
{
    // OpenFHE >=1.4 expects PublicKey& (non-const). Copy shared_ptr into a mutable lvalue.
    auto pk = publicKey.GetRef();
    return std::make_unique<VectorOfCiphertexts>(
        m_cryptoContextImplSharedPtr->EvalMaxSchemeSwitching(ciphertext.GetRef(),
        pk, numValues, numSlots, pLWE, scaleSign));
}
...
std::unique_ptr<CiphertextDCRTPoly> CryptoContextDCRTPoly::EvalMultMutableByCiphertextAndPlaintext(
    CiphertextDCRTPoly& ciphertext, const Plaintext& plaintext) const
{
    // OpenFHE >=1.4 expects Plaintext& (non-const) for ct×pt mutable ops.
    auto pt = plaintext.GetRef();
