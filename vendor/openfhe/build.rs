fn main()
{
    cxx_build::bridge("src/lib.rs")
        ...
        // Avoid OpenMP on mac by default; Homebrew libomp isn't always installed.
    ...
    // linking OpenFHE (prefer cargo link directives over raw -Wl/-l args)
    println!("cargo:rustc-link-search=native=/usr/local/lib");
    println!("cargo:rustc-link-search=native=./openfhe-development/install/lib"); // GitHub Actions
    ...
    // Note: OpenMP is optional; if your OpenFHE build depends on libomp, install it and add:
    // - cargo:rustc-link-search for the libomp directory
