...
/// PVUGC witness extraction hooks
#[cfg(feature = "pvugc-hooks")]
pub trait FriHooks<E: FieldElement> {
    fn on_new_layer(&mut self, _beta: E) {}
    fn on_fold(&mut self, _layer_idx: usize, _q: usize, _v_lo: E, _v_hi: E, _v_next: E, _beta: E) {}
}
    ...
    V: VectorCommitment<H>,
{
    ...
    pub fn verify(
        ...
        match folding_factor {
            2 => self.verify_generic::<2>(channel, evaluations, positions, #[cfg(feature = "pvugc-hooks")] None),
            4 => self.verify_generic::<4>(channel, evaluations, positions, #[cfg(feature = "pvugc-hooks")] None),
            8 => self.verify_generic::<8>(channel, evaluations, positions, #[cfg(feature = "pvugc-hooks")] None),
            16 => self.verify_generic::<16>(channel, evaluations, positions, #[cfg(feature = "pvugc-hooks")] None),
            _ => Err(VerifierError::UnsupportedFoldingFactor(folding_factor)),
        }
    }

    /// Verify FRI proof with PVUGC witness extraction hooks
    #[cfg(feature = "pvugc-hooks")]
    pub fn verify_with_hooks(
        &self,
        channel: &mut C,
        evaluations: &[E],
        positions: &[usize],
        hooks: Option<&mut dyn FriHooks<E>>,
    ) -> Result<(), VerifierError> {
        if evaluations.len() != positions.len() {
            return Err(VerifierError::NumPositionEvaluationMismatch(
                positions.len(),
        ...
        // static dispatch for folding factor parameter (with hooks)
        let folding_factor = self.options.folding_factor();
        match folding_factor {
            2 => self.verify_generic::<2>(channel, evaluations, positions, hooks),
            4 => self.verify_generic::<4>(channel, evaluations, positions, hooks),
            8 => self.verify_generic::<8>(channel, evaluations, positions, hooks),
            16 => self.verify_generic::<16>(channel, evaluations, positions, hooks),
            _ => Err(VerifierError::UnsupportedFoldingFactor(folding_factor)),
        }
    }
    ...
    fn verify_generic<const N: usize>(
        ...
        evaluations: &[E],
        positions: &[usize],
        #[cfg(feature = "pvugc-hooks")] mut hooks: Option<&mut dyn FriHooks<E>>,
    ) -> Result<(), VerifierError> {
        ...
        for depth in 0..self.options.num_fri_layers(self.domain_size) {
            ...
            // PVUGC hooks: log FRI fold data per query (BEFORE computing next evaluations)
            #[cfg(feature = "pvugc-hooks")]
            if let Some(h) = hooks.as_deref_mut() {
                // Announce new layer with its challenge (circuit calls this "beta")
                h.on_new_layer(alpha);

                // Rebuild unique folded positions in same order as layer_values
                let mut unique_folded = folded_positions.clone();
                unique_folded.sort_unstable();
                unique_folded.dedup();

                // Helper: map folded position -> row index in layer_values/row_polys
                let find_row = |fp: usize| -> usize {
                    unique_folded
                        .binary_search_by(|&x| {
                            if x < fp {
                                Ordering::Less
                        ...
                        .expect("pvugc-hooks: folded position missing in unique set")
                };

                // Emit (v_lo, v_hi, v_next, alpha) for each query
                // NOTE: Assumes binary FRI (N == 2)
                for (q_idx, &fp) in folded_positions.iter().enumerate() {
                    let row_idx = find_row(fp);

                    // layer_values[row_idx] is the row (length N) for this folded position
                    // With N == 2: (v_lo, v_hi)
                    let row = &layer_values[row_idx];
                    debug_assert!(
                        row.len() == 2,
                        "pvugc-hooks: expected N==2 (binary FRI)"
                    );
                    let v_lo = row[0];
                    let v_hi = row[1];

                    // v_next is the row polynomial evaluated at alpha
                    let v_next = polynom::eval(&row_polys[row_idx], alpha);

                    h.on_fold(depth, q_idx, v_lo, v_hi, v_next, alpha);
                }
            }
