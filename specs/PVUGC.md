...
* **Fixed outer verifier (MUST).** PVUGC only arms a single, audited Groth16 verifier circuit (the “inner-proof checker”). Its verifying key $\mathsf{vk}$ and CRS hash are pinned inside `ctx_hash`/`GS_instance_digest`, and every instance uses that same circuit. Deployments MUST reject any attempt to arm a different VK.
...
**Note:** Public inputs are bound via the C-matrix constraint `1 * reconstructed = x_pub`, setting $v_{pub} = 0$. Statement binding is through the W-polynomial: $IC_i = w_i/\gamma$.
...
* `"PVUGC/AD_CORE"`, `"PVUGC/DEM/keystream"`, `"PVUGC/DEM/tag"`, `"PVUGC/DEM-Poseidon-v1"`
...
**Fixed outer circuit + QAP audit (MUST).** PVUGC only ever arms a **single, fixed Groth16 outer verifier circuit** (the “inner-proof checker”). The verifying key $\mathsf{vk}$ that appears in this section is that audited outer VK, and its hash is pinned inside `ctx_hash`/`GS_instance_digest`. Before shipping the CRS, implementers MUST extract the outer circuit’s QAP polynomials $\{u_i(X),v_i(X),w_i(X)\}$ and run an offline algebraic audit (e.g. Gröbner-basis reduction) to confirm that, for every allowed public input $x$, the “public residual”
$$
\Delta_{\text{pub}}(X) := W_{\text{pub}}(X) - U_{\text{pub}}(X)V_{\text{pub}}(X)
$$
does **not** lie in the ideal generated by $\{v_j(X) : j>\ell\}$ together with the vanishing polynomial $t(X)$. This check ensures that no adversary can synthesize $R(\mathsf{vk},x)^\rho$ by combining the published witness columns with the Groth16 $H$-queries, even when $\delta_2^\rho$ is public. The audit MUST be bound to the CRS hash; arming MUST refuse any VK/CRS that does not match the audited hash.

**Lean CRS (MUST - Security Hardening).** The "mix-and-match" attack (constructing $\Delta_{\text{pub}}$ via linear algebra) becomes trivial if the adversary has access to raw Powers of Tau ($[\tau^k]_1$) in the CRS. Therefore, **PVUGC deployments MUST NOT distribute or use a standard Groth16 CRS containing raw Powers of Tau.** Instead, they MUST use a **"Lean CRS"** containing only:
1. The fixed verification key $\mathsf{vk}$.
2. The circuit-specific proving key elements ($A_i, B_i, L_i$).
3. **Pre-computed bases** for the quotient polynomial $H(x)$ specific to this fixed circuit (replacing the generic PoT).
This strips the "Universal Solvent" capabilities from the adversary, restricting their subspace back to the specific handles analyzed in the audit, thus restoring the validity of the "One-Sided" security proof.

**Y-basis content (normative):** The $Y_j$ list for B includes:
- $Y_0 = [\beta]_2 + [b_0]_2$ (constant; with $v_{pub}=0$, no public input terms)
- $Y_{j+1} = \text{b\_g2\_query}[1 + \ell + j]$ for every witness-only row used in $B$

The $Y_j$ are **statement-independent** (constant across statements for the same VK) because public inputs are in the C-matrix ($v_{pub}=0$). Statement dependence is isolated to $R(\mathsf{vk},x)$.
...
**Column-based arming:** Directly arm each VK-derived column:
$$
D_0 = ([\beta]_2 + [b_0]_2)^\rho,\quad \{D_{j} = \text{b\_g2\_query}[1+\ell+(j-1)]^\rho\}_{j=1}^{n_B-1}, \quad D_\delta = [\delta]_2^\rho
$$
Column mode directly arms each base. All armed bases are **statement-independent**.
...
**Critical property**: The $Y_j$ and $[\delta]_2$ are **statement-independent** (depend only on VK, not on $x$ or the proof). Statement dependence is isolated to $R(\mathsf{vk},x)$. For *every* valid attestation and fixed $(\mathsf{vk},x)$, the product equals the **same** $R(\mathsf{vk},x)$.
...
**Independence (MUST).** The bases $\{Y_j\}$ and $[\delta]_2$ are fixed by $\mathsf{vk}$ before any armer chooses randomness (they are **statement-independent**). The target $R(\mathsf{vk},x)$ is fixed by ($\mathsf{vk}$, $x$) and carries all statement dependence. Armers cannot choose or influence these values.
...
**γ₂ exclusion enforcement (MUST).** The armed G₂ bases $\{[\beta]_2 + [b_0]_2\} \cup \{\text{b\_g2\_query}[1+\ell+ j]\}$ and $[\delta]_2$ deliberately exclude $[\gamma]_2$. Implementations MUST derive all $Y_j$ from the verifying key's $\{\beta_2, \text{b\_g2\_query (witness columns)}\}$ components only. $[\gamma]_2$ MUST NOT be included in any armed base. This structural exclusion ensures:
- **Statement isolation:** $R(\mathsf{vk},x) = e(\alpha_1,\beta_2) \cdot e(L(x),\gamma_2) + T_{const}(x)$ carries all statement dependence through the $\gamma_2$ factor.
- **Algebraic independence:** The factor $e(L(x),\gamma_2)$ cannot be computed from armed bases, as $[\gamma]_2^\rho$ is never published.
- **Standard hardness:** Computing $R(\mathsf{vk},x)^\rho$ from $\{Y_j^\rho, [\delta]_2^\rho\}$ reduces to DDH in $\mathbb{G}_2$.
...
$$
$$
K_i=\mathrm{SHA\text{-}256}(\mathrm{ser}_{\mathbb{G}_T}(M_i) || H_\text{bytes}(\texttt{ctx\_hash}) || \texttt{GS\_instance\_digest})
$$
...
$$
where $Y_j$, $[\delta]_2$ are **statement-independent** bases derived from VK components and $R(\mathsf{vk},x) = e([\alpha]_1,[\beta]_2) \cdot e(L(x),[\gamma]_2) + T_{const}(x)$ carries all statement dependence. The value is **independent of prover randomization** because the randomness in commitments cancels through the pairing structure.
...
\tilde{M}_i = \left(\prod_{j=0}^{n_B-1} e(X^{(B)}_j,D_j)\right) \cdot e(X^{(B)}_\delta, D_\delta) \cdot e(C,D_\delta) = R(\mathsf{vk},x)^{\rho_i} = M_i
$$
Hence $K_i' = \mathrm{SHA\text{-}256}(\mathrm{ser}_{\mathbb{G}_T}(\tilde{M}_i) || H_\text{bytes}(\texttt{ctx\_hash}) || \texttt{GS\_instance\_digest}) = K_i$, and the DEM decrypts $(s_i \| h_i)$.
...
> Assumption (GT‑XPDH: External Power in $\mathbb{G}_T$). Let $e:\mathbb{G}_1\times\mathbb{G}_2\to\mathbb{G}_T$ be a non‑degenerate bilinear map over prime‑order groups of order $r$. Sample statement-independent base sets $\{Y_j\}, [\delta]_2 \subset\mathbb{G}_2$ (derived from VK), an unknown exponent $\rho\leftarrow\mathbb{Z}_r^*$, and the target $R\leftarrow\mathbb{G}_T$. Given $(\{Y_j\},[\delta]_2,\{Y_j^{\rho}\},[\delta]_2^{\rho},R)$, it is hard for any PPT adversary to compute $R^{\rho}$ without valid commitments. In our instantiation, $R=R(\mathsf{vk},x)$ is fixed by ($\mathsf{vk}$,$x$) and carries statement dependence; the PPE constraint prevents producing commitments without knowledge of a valid Groth16 proof.
...
**Transparent CRS:** Groth–Sahai commitments use a **binding CRS (two G₁ bases)** which we derive deterministically via hash-to-curve from the VK/ctx. No trapdoor or ceremony is required. The G₂ right-legs $\{Y_j\}, [\delta]_2$ are **statement-independent** (derived from VK only). The target $R(\mathsf{vk},x)$ carries all statement dependence. Column-based arming directly arms each VK-derived column. 
...
* DEM correctness (external): $\texttt{ct}_i = (s_i\|h_i) \oplus \mathrm{Poseidon2}(\text{"PVUGC/DEM/keystream"} || K_i, \texttt{ad\_digest}, \text{ctr}_\text{le})$ and $\tau_i = \mathrm{Poseidon2}(\text{"PVUGC/DEM/tag"} || K_i, \texttt{ad\_digest}, \texttt{ct}_i)$ where $\texttt{ad\_digest} = \mathrm{Poseidon2}(\text{"PVUGC/AD\_CORE} || \text{len(AD\_core)}_\text{le} || \text{AD\_core})$.
...
* **Hash‑only DEM (Poseidon2)**: $\texttt{ct}_i=(s_i \| h_i)\oplus \mathrm{Poseidon2}(\text{"PVUGC/DEM/keystream"} || K_i,\texttt{ad\_digest},\text{ctr}_\text{le})$, $\tau_i=\mathrm{Poseidon2}(\text{"PVUGC/DEM/tag"} || K_i,\texttt{ad\_digest},\texttt{ct}_i)$ with $\texttt{ad\_digest} = \mathrm{Poseidon2}(\text{"PVUGC/AD\_CORE"} || \text{len(AD\_core)}_\text{le} || \text{AD\_core})$.
...
$$
$$
M_i = R(\mathsf{vk},x)^{\rho_i} \in \mathbb{G}_T, \quad K_i = \text{SHA\text{-}256}(\text{ser}_{\mathbb{G}_T}(M_i) || H_\text{bytes}(\texttt{ctx\_hash}) || \texttt{GS\_instance\_digest})
$$
...
$$
$$
K_i' = \text{SHA\text{-}256}(\text{ser}_{\mathbb{G}_T}(\tilde{M}_i) || H_\text{bytes}(\texttt{ctx\_hash}) || \texttt{GS\_instance\_digest})
$$
