...
fuzz_target!(|data: &[u8]| {
    ...
    if commitments.c_rows.len() < 2 { return; }
    let a = small(take_bytes::<1>(&mut data).unwrap_or([0])[0]);
    let b = small(take_bytes::<1>(&mut data).unwrap_or([0])[0]);
    if a == 0 && b == 0 { return; }

    use ark_ec::CurveGroup;
    let (c0a, c0b) = commitments.c_rows[0];
    let (c1a, c1b) = commitments.c_rows[1];
    ...
    commitments.c_rows[0] = (c0a_new, c0b_new);
    ...
    for (c_limb0, _) in &commitments.c_rows { x_agg += c_limb0.into_group(); }
    ...
    if OneSidedPvugc::verify(&bundle_forged, &pvugc_vk, &vk, &[x], &gamma) {
