...
fuzz_target!(|data: &[u8]| {
    ...
    for i in 0..k {
        ...
        if coeff == 0 { continue; }
        let (a, b) = commitments.c_rows[i];
        acc0 += a.into_group() * Fr::from(coeff);
        acc1 += b.into_group() * Fr::from(coeff);
    }
    commitments.c_rows[0] = (acc0.into_affine(), acc1.into_affine());
