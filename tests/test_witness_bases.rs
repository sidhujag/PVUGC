...
mod tests {
    ...
    use arkworks_groth16::outer_compressed::{DefaultCycle, InnerE, InnerFr, InnerVk, OuterCircuit, RecursionCycle};
    use std::sync::Arc;
    ...
    impl ConstraintSynthesizer<InnerFr> for SimpleMulCircuit {
        fn generate_constraints(self, cs: ConstraintSystemRef<InnerFr>) -> Result<(), SynthesisError> {
            let x_var = FpVar::new_witness(cs.clone(), || self.x.ok_or(SynthesisError::AssignmentMissing))?;
    ...
    fn test_compute_witness_bases_correctness() {
        ...
        // x * y = z (all witnesses)
        let circuit_inner = SimpleMulCircuit { x: None, y: None, z: None };
        ...
        let outer_circuit: OuterCircuit<DefaultCycle> = OuterCircuit::new(vk_inner.clone(), dummy_x, dummy_proof.clone());
        ...
        let valid_inner_circuit = SimpleMulCircuit { x: Some(x_val), y: Some(y_val), z: Some(z_val) };
        let valid_inner_proof = Groth16::<InnerE>::prove(&pk_inner, valid_inner_circuit, &mut rng).unwrap();
