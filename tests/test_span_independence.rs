...
fn is_in_span<E: Pairing>(target: E::G2Affine, basis: &[E::G2Affine]) -> bool {
...
fn test_no_cross_circuit_span_attack() {
    ...
    let (pk1, vk1) =
        Groth16::<BW6_761>::circuit_specific_setup(circuit1, &mut rng).expect("Setup 1 failed");
    let (pk2, vk2) =
        Groth16::<BW6_761>::circuit_specific_setup(circuit2, &mut rng).expect("Setup 2 failed");
...
fn test_armed_basis_linear_independence() {
    ...
    let (pk, vk) =
        Groth16::<BW6_761>::circuit_specific_setup(circuit, &mut rng).expect("Setup failed");
    ...
    for i in 0..armed_bases.len() {
        ...
        assert!(
            ...
            "Armed bases are not linearly independent at index {}",
            i
        );
    }
}
