    ...
    A::PublicInputs: ToElements<A::BaseField>,
{
    ...
    // Parse commitments
    let num_fri_layers = air.options().to_fri_options().num_fri_layers(air.lde_domain_size());
    ...
    let _ = air.get_constraint_composition_coefficients::<A::BaseField, _>(&mut public_coin)
    ...
    let _deep_coeffs = air.get_deep_composition_coefficients::<A::BaseField, _>(&mut public_coin)
...
fn full_stark_verifier_smoke() {
    ...
    let query_positions = derive_query_positions::<Rp64_256, _>(
        &proof,
        &air,
        &pub_inputs_fe,
    );
